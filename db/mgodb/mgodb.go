...
// NewDB create new connect
func NewDB(fullconnectstring, dbname string) {
	connection(fullconnectstring, dbname)
}

// connection for mongodb official
func connection(fullconnectstring, dbname string) {
	//fmt.Println("fullconnectstring:", fullconnectstring)
	opt := options.Client()
...
// NewCollectionDB returns a new layer collection
func NewCollectionDB(collection string) *MogoDB {
...
func (mg *MogoDB) UpdateOne(filter, updateDoc interface{}) (*mongo.UpdateResult, error) {
	...
	//return mg.c.ReplaceOne(context.TODO(), filter, updateDoc)
}
...
func (mg *MogoDB) Aggregate(pipeline interface{}) (*mongo.Cursor, error) {
	opts := options.Aggregate()
	opts.SetAllowDiskUse(true)
	return mg.c.Aggregate(context.Background(), pipeline, opts)
}
